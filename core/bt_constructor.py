...
def build_behavior_tree(agent, behavior_tree_xml: str, env_pkg: str):
    ...
    env_pkg : str
        Dotted package path for the scenario environment (e.g., "scenarios.pygame.simple").
...
def _parse_xml_to_bt(xml_node, *, bt_module, mission_bt_module, agent, top_xml_path):
    ...
    if node_type == "SubTree":
        ...
        # Mission-level BT parsing (requires mission_autonomy plugin)
        if subtree_id in ("mission_bt", "MissionBT"):
            if mission_bt_module is None:
                raise ModuleNotFoundError(
                    "[ERROR] mission_bt requested but '<env>.mission_bt_nodes' not found."
                )
            return _parse_xml_to_mission_bt(
            ...
            # Regular subtree uses the same parser
            return _parse_xml_to_bt(
                subtree_root.find("BehaviorTree"),
                bt_module=bt_module,
                mission_bt_module=mission_bt_module,
...
def _parse_xml_to_mission_bt(xml_node, *, bt_module, mission_bt_module, agent, top_xml_path):
    """Parse mission-level BT XML. Used only when mission_autonomy plugin is installed."""
    node_type = xml_node.tag

    children = [_parse_xml_to_mission_bt(child,
                                         bt_module=bt_module,
                                         mission_bt_module=mission_bt_module,
                                         agent=agent,
                                         top_xml_path=top_xml_path) for child in xml_node]

    BTNodeList = getattr(mission_bt_module, "BTNodeList")
    attrib = {k: convert_value(v) for k, v in xml_node.attrib.items()}

    if node_type in BTNodeList.CONTROL_NODES:
        control_class = getattr(bt_module, node_type)  # Control nodes from regular bt_module
        return control_class(node_type, children=children, **attrib)

    elif node_type in BTNodeList.MISSION_DECORATOR_NODES:
        decorator_class = getattr(mission_bt_module, node_type)
        if len(children) != 1:
            raise ValueError(f"[ERROR] Mission decorator '{node_type}' must have exactly 1 child.")
        return decorator_class(node_type, child=children[0], **attrib)

    elif node_type in (BTNodeList.ACTION_NODES + BTNodeList.CONDITION_NODES +
                       BTNodeList.MISSION_ACTION_NODES + BTNodeList.MISSION_CONDITION_NODES):
        action_class = getattr(mission_bt_module, node_type)
        return action_class(node_type, agent, **attrib)
        ...
        raise ValueError(f"[ERROR] Unknown mission behavior node type: {node_type}")
