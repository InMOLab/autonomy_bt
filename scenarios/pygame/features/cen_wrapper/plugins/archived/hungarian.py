"""Archived: original hand-rolled centralised Hungarian (cost formula updated).

The original v1 student implementation, with one change: `compute_weight_value`
now uses the same single-distance cost formula as `dec_hungarian` and the
production `hungarian.py` (`1 / λ^(d / AGENT_SPEED)`, no `task.amount/work_rate`
term). With this formula alignment, the hand-rolled bipartite matcher — still
using `random.shuffle(rows)` for tied-row tie-breaking — matches the scipy-based
production `hungarian.py` 100/100 across 100 seeds. This confirms (a) the
random.shuffle is empirically irrelevant in this setup and (b) the original v1's
divergence (39/100 mismatch) came entirely from the different cost formula, not
from randomness.

Not loaded by any active yaml. Kept for reference / educational comparison.
"""
import time
import random
import numpy as np
...
AGENT_SPEED = 0.5  # matches dec_hungarian / hungarian.py


class Hungarian:
    """Archived hand-rolled centralised Hungarian. Same `decide()` interface
    as the production `hungarian.py`, different bipartite-matching internals.
    """
    ...
    def compute_weight_value(self, agent, task):
        ...
        # Aligned with `hungarian.py` (scipy) and `dec_hungarian` — single-distance
        # formula instead of v1's original `d/max_speed + amount/work_rate`.
